...
while True:
    ...
    if option == '1': #Transfer
        ...
        # #Upon receiving replies AND if your block is head of list in block chain
        while True:
            if s1Response == True and s3Response == True and (int(c.ClientBlockChain.getHeadOfRequests().transaction.sender) == int(clientNumber)):
                break
         #Now can request from server
        server.sendall(','.join([option,desiredClient,amount,str(CLK)]).encode()) #sends str(option,clientNum,amn,CLK)
        ...
        CLK+=1
        print("Clock now ",CLK)
        #Maybe inform other clients to update theirHeadOfRequests by one?
